...
# Windows sistem sesleri için (Yalnızca Windows)
try:
    import winsound
except ImportError:
    winsound = None
...
class AnimatedGauge(tk.Canvas):
    ...
    def _animasyon_adimi(self):
        ...
        if abs(fark) > 0.5:
            self.mevcut_aci += fark * 0.15  # Adım hızı (Smooth easing)
...
# ==========================================
# SAF TKINTER GEÇMİŞ GRAFİK MOTORU
# ==========================================
...
class HealthDashboardApp(tk.Tk):
    ...
    def arayuz_kur(self):
        ...
        self.lbl_skor_badge = tk.Label(top_bar, text="Sağlık Skoru: --", bg=CARD_BORDER, fg=ACCENT_GREEN, font=("Segoe UI", 10, "bold"), px=10, py=4)
    ...
    def sekme_dashboard_kur(self):
        ...
        # SAĞ PANEL: METRİK KARTLARI & GAUGE
    ...
    def hesapla_pro(self):
        try:
            ...
            if winsound:
                winsound.Beep(1000, 150) # Onay bip sesi
